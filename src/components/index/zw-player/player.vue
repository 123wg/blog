        ...
        <transition name="dis_list">
            <div class="list_box" v-if="listIsDis">
                ...
                <div class="music_list">
                    <div class="list_l">
                        <ul class="music_type">
                            <li v-for="item in musicTypeList" @click="getMusicType(item.id)" :class="{type_active:item.id==thisMusicType}" :key="item.id">{{item.name}}</li>
                        </ul>
...
import $ from 'jquery';
import {
    getWords, getMusicInfo, getMusicUrl, getHotMusic, getSearchSuggest, getHotTalk,
} from './api/music';
    ...
    methods: {
        ...
        getInfo() {
            getMusicUrl(this.musicList[this.thisMusicIndex].id).then((res) => {
                if (res.data.data[0].url == null || res.data.data[0].url === '' || res.data.data[0].url === undefined) {
                    ...
                    getWords(this.musicList[this.thisMusicIndex].id).then((ress) => {
                        if (!ress.data.nolyric) {
                            const info = this.Cut(ress.data.lrc.lyric);
                            this.musicWords = info.wordArr;
                            this.wordsTime = info.timeArr;
                        }
                    });
                    getHotTalk(this.musicList[this.thisMusicIndex].id).then((resss) => {
                        let count = 0;
                        this.hotTalkList = resss.data.hotComments.splice(0, 3);
        ...
        Cut(str) {
            ...
            for (let i = 0; i < timeArr.length; i += 1) {
                ...
                timeArr[i] = Number(time[0]) * 60 + Number(time[1]);
        ...
        Player() {
            ...
            function timer() {
                ...
                if (player.currentTime >= self.wordsTime[self.o + 1]) {
                    self.top += Number($('.music_word').eq(self.o).height() + Number($('.music_word').eq(self.o).css('marginTop')));
            ...
            $('.progress').on('mousedown', (ev) => {
                ...
                $(document).on('mouseup', () => {
                    ...
                    if (this.o < now_o) {
                        // eslint-disable-next-line camelcase
                        for (let i = this.o; i < now_o; i += 1) {
                            // eslint-disable-next-line camelcase
                            diff_h += -Number($('.music_word').eq(i).height() + Number($('.music_word').eq(i).css('marginTop')));
                        }
                    } else {
                        // eslint-disable-next-line camelcase
                        for (let i = now_o; i < this.o; i += 1) {
                            // eslint-disable-next-line camelcase
                            diff_h += Number($('.music_word').eq(i).height() + Number($('.music_word').eq(i).css('marginTop')));
                        }
                    }
